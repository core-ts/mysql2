            ...
            connection.execute(statements[0].query, toArray(statements[0].params), function (er2a, results0) {
              if (er2a) {
                ...
                if (results0 && results0.affectedRows === 0) {
                  ...
                  connection.execute(queries_1.join(""), toArray(params_1), function (er2, results) {
                    if (er2) {
                      connection.rollback(function () {
                        return reject(er2);
                      });
      ...
      pool.getConnection(function (er0, connection) {
        ...
        connection.beginTransaction(function (er1) {
          if (er1) {
            ...
            connection.execute(queries_2.join(""), toArray(params_2), function (er2, results) {
              if (er2) {
                connection.rollback(function () {
                  buildError(er2);
                  return reject(er2);
          ...
          connection.execute(statements[0].query, toArray(statements[0].params), function (er2a, results0) {
            if (er2a) {
              ...
              if (results0 && results0.affectedRows === 0) {
                ...
                connection.execute(queries_3.join(""), toArray(params_3), function (er2, results) {
                  if (er2) {
                    connection.rollback(function () {
                      return reject(er2);
                    });
    ...
    return new Promise(function (resolve, reject) {
      connection.beginTransaction(function (er1) {
        if (er1) {
          ...
          connection.execute(queries_4.join(""), toArray(params_4), function (er2, results) {
            if (er2) {
              connection.rollback(function () {
                buildError(er2);
                return reject(er2);
      ...
      connection.execute(statements[0].query, toArray(statements[0].params), function (er2a, results0) {
        if (er2a) {
          ...
          if (results0 && results0.affectedRows === 0) {
            ...
            connection.execute(queries.join(""), toArray(params), function (er2, results) {
              if (er2) {
                connection.rollback(function () {
                  return reject(er2);
                });
